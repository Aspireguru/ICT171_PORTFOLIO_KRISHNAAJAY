...
Double-check that the A record is correctly added on Namecheap with the exact host portfolio and correct IP. Always remember it can take 30-1 hour for DNSA change to be globally acknowledged. Use tools like whatsmydns.net to see if DNS has propagated in various regions.
...
### Error messages
403 Forbidden(Nginx could not read the file). Make sure the file exists in the correct location and permissions are not restrictive.  chmod 755 on the web directory should prevent this by giving world-read access. If you set different permissions, ensure that at least the www-data user can read the files.
...
The firewall is causing issues with the firewall settings. If UFW is enabled but only Nginx HTTP was allowed, after enabling HTTPS, you should also allow Nginx HTTPS, or we should switch to combined profiles. We can do this by: 
